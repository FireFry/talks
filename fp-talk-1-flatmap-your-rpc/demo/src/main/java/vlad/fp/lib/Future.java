...
public abstract class Future<T> {
  ...
  public static <T> Future<T> apply(Supplier<T> a, ExecutorService pool) {
    return Async(voidF(cb -> pool.submit(Utils.logErrors(() -> cb.apply(a.get()).run()))));
  }
  ...
  public static <T> Future<T> schedule(Supplier<T> supplier, Duration delay, ScheduledExecutorService pool) {
    return Async(Utils.voidF(cb -> pool.schedule(
        () -> cb.apply(supplier.get()).run(), delay.toMillis(), TimeUnit.MILLISECONDS)));
  }
  ...
  public Future<T> step() {
    return Tailrec.run(this, x -> x.matchT(
        Tailrec::finish,
        suspend -> Tailrec.next(suspend.thunk.get()),
        Tailrec::finish,
        bindSuspend -> Tailrec.next(bindSuspend.thunk.get().flatMap(bindSuspend.f)),
        Tailrec::finish
    ));
  }
